...
def safe_output_path(output, name):
    # Reject path traversal / absolute / drive-relative entry names (from a
    # crafted archive or the .lst name list) so extraction can't write outside
    # the chosen output directory (zip-slip).
...
def read_hash(path, body=str(), output=str(), namepath=str(), algo=str(), threshold=70):
    ...
    with open(body if body else path, "rb") as file:
        for hash, offs, size in sorted(zip(hash_list, offs_list, size_list), key=lambda x: x[1]):
            ...
            outpath = safe_output_path(output, name)
            file.seek(offs)
