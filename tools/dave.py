...
TOOL_VERSION = "v1.5.3"
TOOL_AUTHOR = "Written by Edness"


def build_tool_stamp(default_name="dave.py"):
    tool_name = os.path.split(sys.argv[0])[-1] or default_name
    return f"{tool_name} - {TOOL_VERSION} - {TOOL_AUTHOR}".encode("UTF-8")
...
def safe_output_path(output, name):
    # Reject path traversal / absolute / drive-relative entry names so a crafted
    # archive can't write outside the chosen output directory (zip-slip).
    base = os.path.abspath(output)
    target = os.path.abspath(os.path.join(base, name))
    if target != base and not target.startswith(base + os.sep):
        raise ValueError(ERR_TRAVERSE.format(name))
    return target
...
def read_dave(path, output=str()):
    ...
    with open(path, "rb") as file:
        ...
        for i in range(entries):
            ...
            outpath = safe_output_path(output, file_name)
...
ERR_TRAVERSE = "Error! Refusing to write outside the output directory. ({})"
